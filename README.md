...
go run ./cmd/ch09-rag 'Chapter 7 streaming은 RAG와 어떻게 연결될 수 있나요?'
```

두 번째 예시는 `testdata/docs/ch09-rag/chapter07-streaming.md` 문서를 검색해 RAG의 retrieval 결과를 후속 streaming 응답으로 연결할 수 있다는 개념을 설명합니다. 현재 Chapter 09 CLI는 streaming 출력이 아니라 source-grounded final answer 출력에 집중합니다.
